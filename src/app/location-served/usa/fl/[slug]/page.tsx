...
import LocationServedUsaFl from "@/components/templates/LocationServedUsaFl";
import { fetchPageData, fetchInterlinking } from "@/lib/wordpress";

export const revalidate = 60;
export const dynamicParams = true;

const BASE_PATH = "location-served/usa/fl";
...
export async function generateStaticParams() {
  ...
  try {
    const res = await fetch(
      `${wpUrl}/wp-json/builder/v1/pages?path=%2Flocation-served%2Fusa%2Ffl`,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!data) notFound();
  return <LocationServedUsaFl fields={data.fields} relatedLinks={relatedLinks} />;
}
