...
import LocationServedUsaCaFountainValley from "@/components/templates/LocationServedUsaCaFountainValley";
import { fetchPageData, fetchInterlinking } from "@/lib/wordpress";

export const revalidate = 60;
export const dynamicParams = true;

const BASE_PATH = "location-served/usa/ca/fountain-valley";
...
export async function generateStaticParams() {
  ...
  try {
    const res = await fetch(
      `${wpUrl}/wp-json/builder/v1/pages?path=%2Flocation-served%2Fusa%2Fca%2Ffountain-valley`,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!data) notFound();
  return <LocationServedUsaCaFountainValley fields={data.fields} relatedLinks={relatedLinks} />;
}
