...
const BASE_PATH = "location-served/usa/ca/newport-beach";
...
export async function generateStaticParams() {
  ...
  try {
    const res = await fetch(
      `${wpUrl}/wp-json/builder/v1/pages?path=%2Flocation-served%2Fusa%2Fca%2Fnewport-beach`,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!data) notFound();
  return <LocationServedUsaCaNewportBeach fields={data.fields} relatedLinks={relatedLinks} />;
}
