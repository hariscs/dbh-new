...
import LocationServedUsaCa from "@/components/templates/LocationServedUsaCa";
...
export async function generateStaticParams() {
  ...
  try {
    const res = await fetch(
      `${wpUrl}/wp-json/builder/v1/pages?path=%2Flocation-served%2Fusa%2Fca`,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!data) notFound();
  return <LocationServedUsaCa fields={data.fields} relatedLinks={relatedLinks} />;
}
