...
const BASE_PATH = "location-served/usa/lisdexamfetamine-vyvanse";

const FALLBACK_SLUGS = ["withdrawal","high-dose-signs","uses-risks"] as string[];
...
export async function generateStaticParams() {
  ...
  try {
    const res = await fetch(
      `${wpUrl}/wp-json/builder/v1/pages?path=%2Flocation-served%2Fusa%2Flisdexamfetamine-vyvanse`,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      ...
      <LocationServedUsaLisdexamfetamineVyvanse {...data.fields} createdAt={data.meta.createdAt} />
    </>
  );
}
