...
const BASE_PATH = "location-served/usa/tn/middle";
...
export async function generateStaticParams() {
  ...
  try {
    const res = await fetch(
      `${wpUrl}/wp-json/builder/v1/pages?path=%2Flocation-served%2Fusa%2Ftn%2Fmiddle`,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const data = await fetchPageData(`${BASE_PATH}/${slug}`);
  if (!data) notFound();
  return <LocationServedUsaTnMiddle {...data.fields} />;
}
