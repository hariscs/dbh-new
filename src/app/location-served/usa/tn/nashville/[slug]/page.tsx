...
import LocationServedUsaTnNashville from "@/components/templates/LocationServedUsaTnNashville";
import { fetchPageData, fetchInterlinking } from "@/lib/wordpress";

export const revalidate = 60;
export const dynamicParams = true;

const BASE_PATH = "location-served/usa/tn/nashville";
...
export async function generateStaticParams() {
  ...
  try {
    const res = await fetch(
      `${wpUrl}/wp-json/builder/v1/pages?path=%2Flocation-served%2Fusa%2Ftn%2Fnashville`,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!data) notFound();
  return <LocationServedUsaTnNashville fields={data.fields} relatedLinks={relatedLinks} />;
}
