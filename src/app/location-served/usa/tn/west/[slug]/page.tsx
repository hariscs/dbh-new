...
import LocationServedUsaTnWest from "@/components/templates/LocationServedUsaTnWest";
...
export async function generateStaticParams() {
  ...
  try {
    const res = await fetch(
      `${wpUrl}/wp-json/builder/v1/pages?path=%2Flocation-served%2Fusa%2Ftn%2Fwest`,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!data) notFound();
  return <LocationServedUsaTnWest fields={data.fields} relatedLinks={relatedLinks} />;
}
