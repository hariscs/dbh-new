...
/**
 * The nav as it stood before it was driven by WordPress: a verbatim snapshot of the
 * rendered mega menu, kept as the fallback PrimaryNav uses when `fetchMenu()` returns
 * null. The nav is on every page, so a WordPress outage or a broken payload must
 * degrade to this rather than to an empty header.
 *
 * Do not hand-edit to change menu content — that lives in WP Admin now. This exists
 * only so the site still has a usable nav when the CMS cannot be reached.
 */
export default function PrimaryNavStatic({ idSuffix = "", withBack = false }: { idSuffix?: string; withBack?: boolean }) {
  return (
    <>
